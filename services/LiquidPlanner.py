...
def build_tasks(json: List[dict]) -> List[Task]:
    return [Task(task['id'], task['name'], task['package_id'], build_assignments(task['assignments'])) for task in json]
...
def fetch_tasks_by_package(package_id: int) -> List[Task]:
    ...
    response = fetch(FETCH_TASKS_URL, query_params)
    if response.status_code != 200:
        raise Exception(f'Response Error: {response.text}')
...
def fetch_task_snapshots(task_id: int) -> List[Snapshot]:
    response = fetch(FETCH_TREEITEM_SNAPSHOTS_URL_FORMAT.format(task_id))
    if response.status_code != 200:
        raise Exception(f'Response Error: {response.text}')
